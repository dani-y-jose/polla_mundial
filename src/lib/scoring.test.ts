...
function runTests() {
  ...
  const scoresA = calculateGroupScores(members, matchesA, predictionsA, rules);
  ...
  const scoresB = calculateGroupScores(members, matchesA, predictionsB, rules);
  ...
  const scoresC = calculateGroupScores(["user1", "user2"], roundOf16Matches, predictionsC, rules);
