...
// WhatsApp share link — dedupes the icon + styling. The caller builds the
// `message` (it's group-specific); this owns the brand button shell and the
// share URL. `fullWidth` (default) = bloque; `fullWidth={false}` = inline compacto.
...
export function WhatsAppShareButton({
  ...
  return (
    <a
      ...
      className={cn(
        "inline-flex items-center justify-center gap-2 rounded-xl bg-[#25D366] font-extrabold text-[#0b3d20] transition-colors hover:bg-[#1eb455]",
        fullWidth ? "w-full py-2.5 text-sm" : "min-h-[36px] px-4 text-xs",
        className,
      )}
    >
